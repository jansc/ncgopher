...
impl History {
    ...
    pub fn add(&mut self, entry: HistoryEntry) {
        ...
        if let Ok(_) = self.sql.query_row(
            "SELECT id FROM history WHERE url=?1",
            params![&entry.url],
            |_| Ok(())
        ) {
            trace!("History::add(): Row exists, updating");
            let mut stmt = self.sql.prepare("UPDATE history SET visitedcount=visitedcount+1 WHERE url=?1").expect("Could not update history entry");
            ...
            let last_id: String = self.sql.last_insert_rowid().to_string();
            info!("History::add(): Last entry id {}", last_id); 
        }
    }
    ...
    pub fn get_latest_history(&self, num_items: usize) -> Vec<HistoryEntry> {
        ...
        info!("Getting {} history entries", num_items as u32);

        let mut stmt = self.sql.prepare("SELECT title, url, timestmp, visitedcount FROM history ORDER BY timestmp LIMIT ?1").expect("Could not get history entry");
        let mut rows = stmt.query(params![num_items as u32]).expect("Could not execute query");
        while let Some(row) = rows.next().expect("Could not read rows") {
            info!("Found history entry");
        ...
        info!("Returning {} history entries", res.len());
        res
    }
}
